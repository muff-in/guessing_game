...
fn main() {
    ...
    loop {
        ...
        let guess: u32 = match guess.trim().parse() { // .trim is used to remove whitespace from the input and .parse() is used to convert the input to a number
            Ok(num) => num, // Ok() is used to handle successful conversions
            Err(_) => { // Err() is used to handle unsuccessful conversions and _ is used to ignore the error
        ...
        match guess.cmp(&secret_number) { // cmp() is used to compare two numbers
            ...
            Ordering::Equal => { // Ordering::Equal is used to handle numbers that are equal to the secret number
                println!("You win"); 
